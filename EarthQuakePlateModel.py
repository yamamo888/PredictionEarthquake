...
class EarthQuakePlateModel:
    ...
    def plotVWavelet(self):
        ...
        #self.yV:10000
        #cwtmatr:(widths,self.V[:,figInd])
        for figInd in np.arange(len(figInds)):          
            #cwtmatr, freqs = pywt.cwt(self.V[:,2+figInd], widths, 'mexh')
            cwtmatr, freqs = pywt.cwt(self.yV[:,figInd], widths, 'mexh')
            period = 1.0 / freqs
            
            #extent＝[左下、右下、左下、左上]：表示範囲
            figInds[figInd].imshow(cwtmatr, extent=[0, 10000, 0, max(period)], cmap='PRGn', aspect='auto',vmax=abs(cwtmatr).max(), vmin=-abs(cwtmatr).max())
