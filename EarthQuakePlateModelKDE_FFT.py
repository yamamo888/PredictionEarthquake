...
class Data:
	...
	def FFT(self,widthWindow=25,eFrq=250,nYear=8000,nCell=8,trainRatio=0.8,isTensorflow=False):
		...
		nSlice = int(len(self.fft_amp[0,0,:])/widthWindow) 
		
		
		#　FFTをかけたデータをsizeで分割する			
		sliceflag = False
		for cnt in np.arange(nSlice):
			sInd = widthWindow * cnt
			eInd = sInd + widthWindow
			...
			if not sliceflag:
				self.yVfft = fft_slice_mean
				sliceflag = True
			else:
				self.yVfft = np.concatenate((self.yVfft,fft_slice_mean),axis=2) #[nData,nCell,nSlice]
		...
		# 学習データ
		self.xTrain = self.yVfft[self.randInd[0:self.nTrain]]
		self.yTrain = self.Y[self.randInd[0:self.nTrain]]
		
	   
		# 評価データ
		self.xTest = self.yVfft[self.randInd[self.nTrain:]]
		self.yTest = self.Y[self.randInd[self.nTrain:]]
		'''
